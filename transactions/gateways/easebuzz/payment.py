...
def _checkArgumentValidation(*arg):
    if len(arg) != 4:
        return {"status": 0, "data": "Invalid number of arguments."}
    return True
...
def _typeValidation(params):
    ...
    if type_value != False:
        return {"status": 0, "data": type_value}
...
def _email_validation(email):
    if not re.match(r"^([\w\.-]+)@([\w-]+)\.([\w]{2,8})(\.[\w]{2,8})?", email):
        return {"status": 0, "data": "Email invalid, Please enter valid email."}
    return True
...
def _pay(params_array, salt_key, url):
    ...
    if not accesskey:
        ...
        return {"status": 1, "data": url + "pay/" + accesskey, "access_key": accesskey}
...
def easebuzzResponse(response_params, salt_key):
    if len(response_params) == 0:
        return {"status": 0, "data": "Response params is empty."}
...
def _getResponse(response_array, s_key):
    ...
    if reverse_hash_key == response_array["hash"]:
        if response_array["status"] == "success":
            return {"status": 1, "url": response_array["surl"], "data": response_array}
        elif response_array["status"] == "failure":
            return {"status": 1, "url": response_array["furl"], "data": response_array}
        else:
            return {"status": 1, "data": response_array}
    else:
        return {"status": 0, "data": "Hash key Mismatch"}
